...
<div class="articles">
    <h3>
        <?= ($article['titre']) ?> <em>le <?= $article['date_creation'] ?></em>
    </h3>
...
<div class="formulaire">
    ...
    <div class="commentaires">
        <p><strong><?= htmlspecialchars($commentaire['auteur']) ?></strong> a commenté le
            <?= $commentaire['date_commentaire'] ?>
        </p>

        <p class="commentaireContenu"><?= nl2br(htmlspecialchars($commentaire['contenu'])) ?>
            <br><br>
            <?php if ($commentaire['moderation'] != 1 ) { ?>

            <span class="moderation">
                <a href="./index.php?action=signalCommentaire&id=<?php echo $commentaire['id']; ?>&moderation=<?php echo $commentaire['moderation']; ?>&articleId=<?php echo $article['id']; ?>" onclick="return confirm('Êtes-vous sûr et certain? Cette action est permanente !')">Signaler pour modération</a>
            </span>
